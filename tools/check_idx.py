...
DType = importlib.import_module("utils.dtype").DType
...
def check_idx(idx_path, bin_path):
    ...
    with open(idx_path, "rb") as f:
        ...
        print(
            f"DType code : {dtype_code} (对应: {dtype_type.__name__}, 大小: {dtype_val} bytes)"
        )
    ...
    print(
        "✓ 3. 索引逻辑 (Sizes 和 Pointers 数组机制) 的逻辑是和 Megatron 完全一致的 (前缀和作为偏移量)。"
    )

    print("\n【结论】:")
    print(
        "现在你的 idx 头部结构已经和 Megatron-LM 的标准 `_INDEX_HEADER` 格式高度对齐。"
    )
    print(
        "注意原版 Megatron 可能还会在文件头写入 9字节 的 `MMIDIDX\\x00\\x00`，这取决于 Megatron 的具体版本。"
    )
    print("=====================================\n")
