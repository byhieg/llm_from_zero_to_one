...
class SimpleMegatronDataset(Dataset):
    ...
    def build_dataset_indices(self):
        ...
        cache_base_name = (
            f"{idx_name}_seq{self.seq_len}_seed{self.seed}_tokens{self.total_token}"
        )
        ...
        # 2. 检查缓存是否存在
        if (
            os.path.exists(path_doc_idx)
            and os.path.exists(path_sample_idx)
            and os.path.exists(path_shuffle_idx)
        ):
...
if __name__ == "__main__":
    dataset = SimpleMegatronDataset(
        idx_path="train_data/tiny_stories.idx",
        bin_path="train_data/tiny_stories.bin",
        seq_len=1024,
        total_token=100 * 10000,
        seed=42,
    )
