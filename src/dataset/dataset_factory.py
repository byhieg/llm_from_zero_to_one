...
def create_dataset(
    data_strategy: str, dataset_config: dict[str, Any] = None, **kwargs
) -> Dataset:
    """
    创建数据集实例
...
def _create_padding_dataset(dataset_config: dict[str, Any], **kwargs) -> Dataset:
    ...
    if not tokenizer_path:
        raise ValueError(
            "padding 策略需要在 dataset_config 中提供 'tokenizer_path' 参数"
        )
    ...
    if not dataset_path:
        raise ValueError("padding 策略需要在 dataset_config 中提供 'dataset_path' 参数")
    dataset_name = dataset_config.get("dataset_name") or None
...
def _create_megatron_dataset(dataset_config: dict[str, Any], **kwargs) -> Dataset:
    ...
    if not dataset_path:
        raise ValueError(
            "预先分词策略需要在 data.dataset_config 中提供 'dataset_path' 参数"
        )
